...
8. **Phase 3** — supprimer proprement les vues **Comptes** et **Paramètres** masquées du menu Super Admin (code aujourd’hui conservé) ; implémenter **`defaultOrgId` global** à la racine Firebase (remplace un champ par org sans effet réel homogène dans l’app) ; **« Visible sur calendrier »** par studio (filtrage affichage) ; **gestion `sharedWith`** (UI + règles) ; **hub multi-plannings** (enfants, crèche, etc., réutilisation du socle multi-org)
...
### v4.6.3 — Avril 2026
- **`superadmin.html`** — renommage du **libellé** d’une organisation depuis la vue **Organisations** (bouton **✏️**, modale **Renommer l’organisation** avec ligne **ID base de données** et champ libellé ; écriture **`/organizations/{orgId}`** **`{ label }`**, slug inchangé) ; libellé de champ **ID base de données (slug)** dans la modale de **création** d’organisation ; **slug** non affiché sous le libellé sur la ligne org (visible dans la modale de renommage) ; topbar : une seule ligne **Planning Ménage · v** (fusion des deux sous-titres) ; message discret sur l’écran de login Super Admin (interface non prévue pour smartphone) ; **`APP_VERSION` : 4.6.3**.
- **`index.html`** — topbar **Accueil** sur les écrans de connexion / choix d’organisation / org non configurée ; sous-titre studios vide sur ces écrans ; suppression du bouton cadenas **🔒** : le **texte de synchronisation** ouvre **`superadmin.html`** ; **`APP_VERSION` : 4.6.3**.
- **`admin.html`**, **`compte.html`** — **`APP_VERSION` : 4.6.3**.
