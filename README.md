...
| [Super Administration](https://jonathantesson.github.io/planning-menage/superadmin.html) | Interface Super Admin (login dédié, hors session planning) — aussi via le **texte de synchronisation** en topbar sur le calendrier |
...
- **Login** :
  - **Firebase Authentication** : bouton « Se connecter avec Google » (principal) + email / mot de passe (secondaire).
  - Accès réservé à **`tessonjonathan@gmail.com`** (constante **`SA_ALLOWED_EMAIL`** hardcodée).
...
L’**organisation par défaut** proposée à la connexion dépend de **`defaultOrgId`** dans **`adminConfig`** et de la liste chargée ; la valeur **`tesson`** reste un **défaut** courant dans le code lorsque rien n’est configuré. Depuis **v4.6.2**, il n’y a **plus d’UI** dans **`admin.html`** pour ce champ (réglage via **Firebase** ou **Super Admin** → vue Paramètres, masquée dans le menu mais accessible en **`#parametres`**) ; effet applicatif à consolider en **Phase 3** (voir **Améliorations prévues**).
...
- **`admin.html`** + **`compte.html`** : **`authStateReady()`** + **`auth.currentUser`** → lookup **`authUid`** dans **`/accounts`** → fallback **`localStorage`** session pendant transition.
- **`superadmin.html`** : Firebase Auth Google/email, email autorisé hardcodé (**`SA_ALLOWED_EMAIL = 'tessonjonathan@gmail.com'`**), re-auth sur opérations sensibles.
...
├── invitation.html      → Parcours d'inscription par token (v4.9.6 — 6 étapes : validation token → compte → org → studios → iCal → confirmation)
...
├── migrate-to-firebase-auth.js → Migration one-shot : création des comptes Firebase Auth depuis /accounts
├── migration-passwords.template.js → Template mots de passe temporaires (versionné, sans mots de passe réels)
├── update-accounts-auth.js → Script one-shot : mise à jour emails + suppressions dans Firebase Auth et RTDB
├── update-accounts-auth.template.js → Template des opérations (versionné)
├── init-accounts.js     → Init one-shot : compte de test sous /accounts (SHA-256) (remplacé par migrate-to-firebase-auth.js pour les migrations Firebase Auth)
...
Sous **`/orgs/{orgId}/assignments/{uidRéservation}`**, l’objet d’assignation comporte en pratique **`c1`**, **`c2`** (ou format historique : valeur chaîne unique pour une seule intervenante), **`note`** (texte libre, ex. consigne clé), depuis la v4.3.4 **`noteBy`**, et depuis la **v4.10.0** **`menageDate`** / **`menageDateBy`** :
...
| `menageDate` | **v4.10.0** — date **`YYYY-MM-DD`** (locale) à laquelle le **ménage** doit réellement avoir lieu, quand elle diffère de la date de départ (ex. pas de réservation juste après, ou regroupement de deux ménages). `null` / absent = pas de décalage, le ménage a lieu le jour du départ (comportement historique, inchangé). Saisie dans la modale **Assigner les intervenantes** sur **`index.html`** (bouton **🧹 Décaler l’intervention de ménage**), bornée par un `<input type="date">` natif : `min` = date de départ, `max` = veille de la **prochaine arrivée** sur le studio (**`nextArrivalAfter`**) — dates au‑delà **grisées** dans le sélecteur, et **doublement vérifiées** à l’enregistrement (**`_sa()`**) pour bloquer toute date invalide qui contournerait le `max` (saisie clavier). Une date **égale** à la date de départ équivaut à « pas de décalage » (`null`). |
| `menageDateBy` | Renseigné **uniquement** si `menageDate` est défini : prénom ou libellé d’acteur (**`assignActorLabel()`**) au moment du décalage. `null` si le décalage est annulé (**`✕ Annuler le décalage`**). |

**Date effective** (`menageDate` si présent, sinon date de départ) — fonction `menageEffectiveDate(r)` dupliquée (même pattern que `getAssignment`/`getAssignmentSlots`) sur **`index.html`**, **`compte.html`** et **`admin.html`**, utilisée partout où une intervention est rattachée à un mois ou triée par date : KPIs et historique de **`compte.html`**, statistiques dashboard et **Historique des interventions** + **export Excel** de **`admin.html`** (voir détails § **index.html — Calendrier**, § **compte.html**, § **admin.html** ci-dessous). **`notify-departs.js`** ne change pas de logique de déclenchement (toujours le jour réel du départ) : seul le texte du message Telegram indique le décalage. La Cloud Function **`nextIntervenante`** sélectionne le prochain ménage d'un studio sur cette même **date effective** (pas uniquement `menageDate` du prochain départ brut — corrigé après un bug en Septembre 2026, voir § **functions/index.js**).
...
4. **Comptes** : créer les comptes dans l’**admin** de l’org ou via le **Super Admin** (panneau comptes de la vue **Organisations**) ; en **v4.7.0**, viser aussi une entrée cohérente sous **`/accounts`** (ou exécuter **`patch-legacy-accounts.js`** après création globale) pour que connexion **`index.html`** et **`compte.html`** restent alignés.
...
**Organisation**
- **v4.7.0** — Premier accès (pas de `menage_org_v1` dans `localStorage`) : l’app initialise **`menage_org_v1`** à la **première organisation** renvoyée par **`/organizations`** (ou **`tesson`** en secours), puis charge l’app **sans** afficher l’écran **Organisation d’abord** ; l’écran **`#org-first-screen`** et les fonctions **`showOrgFirst` / `confirmOrgFirst`** restent dans le code pour d’éventuels flux manuels mais ne sont **plus** invoqués au démarrage.
- **Topbar** (**v4.6.3**) : sur les écrans **Organisation**, **Connexion** et **Organisation non configurée**, le titre affiche **Accueil** et le sous-titre studios (**`#topbar-sub`**) est vide ; après **`showApp()`**, titre **Planning Ménage** et sous-titre = noms des studios. Le texte de **synchronisation** (à droite) est un **lien** vers **`superadmin.html`**.
- **Pastilles orange titre** (**v4.6.3**) : à côté du titre après **`showApp()`**, **`#pending-suggestions-pip`** signale encore les suggestions procédure en attente pour **admin** ou **sans auth**. **v4.9.10** : la **même** pastille (**`paintSuggestionsPip`**) signale aussi l’absence sur le **mois civil actuel** d’au moins une réservation avec **nombre de voyageurs valide** (1–8) dans **`assignments`** — mis à jour sur **`assignments`** / **`reservations`** et **`procedureSuggestions`**.
- Connexion (**v4.7.0**) : **Organisation** en premier (**`#login-org`**, **`onLoginOrgChange`** → mise à jour **`menage_org_v1`** + **`sessionStorage`** **`menage_org_change_gate`** + **rechargement** pour attacher Firebase à la bonne org). **Nom** : **`<select id="login-name">`** (libellé **Nom** au-dessus) ; à l’affichage du login, **`showLogin()`** appelle **`get(ref(db,'accounts'))`**, parcourt les lignes, ajoute chaque libellé non vide **`String(row.name).trim() || String(row.prenom).trim()`** dans un **`Set`**, trie en **`fr`**, puis injecte des **`<option value="…">`** après la ligne **`-`** (`value` vide). Mot de passe : **`#login-pwd`**, envoi **`doLogin()`** — d’abord candidats **`/accounts`** dont **`name`** ou **`prenom`** égale la valeur du select et **`pwdHash`** (SHA-256 hex du mot de passe saisi), sinon compte legacy **`adminConfig.accounts`** avec **`hashSimple`**. Tant que le select ne liste que **`/accounts`**, un intervenant **uniquement** legacy sans entrée **`/accounts`** correspondante **n’apparaît pas** dans le menu (aligner avec **`patch-legacy-accounts.js`** ou faire évoluer le remplissage du select).
...
- **Conflit assignation × indispo (v4.9.10)** : si **`c1`** ou **`c2`** est assignée sur un **départ ce jour‑là** (**`getCheckouts`** + même filtre **`filterCleaner`** que l’affichage des lignes départ) et que cette même personne a une entrée **indispo** (**`unavailLocalPresent`**) pour la **date locale** du jour, la **case jour** (**`.day-cell`**) prend la classe **`day-cell--assign-indispo-warn`** (bordure rouge **`#E24B4A`**, survol titre explicatif) — **sans** masquer ou modifier les autres affichages. **Affichée même si le bouton filtres « Indispo » est désactivé**.
- **Ménage décalé (v4.10.0)** : dans la modale **Assigner les intervenantes**, section **Ménage** — bouton **🧹 Décaler l’intervention de ménage** / **🧹 Modifier le décalage** ouvrant un panneau avec un sélecteur de date (`min` = départ, `max` = veille de la prochaine arrivée sur le studio, dates au-delà grisées nativement) et un bouton **✕ Annuler le décalage**. Écrit **`menageDate`** / **`menageDateBy`** sous l’assignation (voir § **`/assignments`**). **Affichage calendrier** : le jour du **départ**, si un décalage existe, les noms d’intervenantes et le point de note sont masqués sur le bloc départ (remplacés par une pastille 🧹) et le bloc prend la classe **`checkout--decaled`** (fond ocre) ; sur le **jour du ménage décalé**, un bloc dédié **`.resa-block.menage`** (« 🧹 Ménage S… », fond bleu clair) affiche les mêmes badges d’intervenantes, filtré comme les autres blocs par **`S.filterCleaner`**. **Légende** : entrée **« 🧹 Ménage décalé »**. **Conflit assignation × indispo** : ignoré sur le jour de départ si un décalage existe (**`departDayHasAssignIndispoConflict`**), vérifié à la place sur le jour du ménage décalé (**`getMenageDecalsForDay`**). Modale (vue ménagère et admin) : ligne **« 🧹 Ménage prévu le … »** sous les dates si un décalage est actif. **Garde-fou** : à l’enregistrement (**`_sa()`**), toute date de ménage tombant sur ou après la prochaine arrivée est refusée (toast d’erreur, rien n’est écrit) — même si elle a contourné le `max` du sélecteur (saisie clavier). Fonctions ajoutées : **`getAssignment`** (champs **`menageDate`**/**`menageDateBy`**), **`rawAssignmentObj`** (fusion préservant tous les champs existants — **`voyageurs`** inclus — lors d’un **M’assigner**/**Me retirer**/assignation admin), **`getMenageDecalsForDay`**, **`fmtShortDate`**, **`nextArrivalAfter`**, **`window._toggleMenageDecal`**, **`window._clearMenageDecal`**.

### compte.html — Espace intervenante

- **Accès** : session **`menage_session_v1`** (prénom) + org **`menage_org_v1`** ; sinon redirection vers **`index.html`**. Comptes **admin** : redirection vers **`index.html`** (page réservée aux intervenantes sans rôle admin). **v4.7.0** : le déverrouillage et le **changement de mot de passe** sur cette page s’appuient encore sur **`adminConfig.accounts`** (legacy) : un utilisateur **uniquement** présent dans **`/accounts`** sans ligne legacy correspondante peut être **redirigé** ou ne pas voir le formulaire MDP tant que le legacy n’est pas aligné (ex. script **`patch-legacy-accounts.js`**).
...
- **Dashboard** : mois (← / libellé / → comme l’admin), KPIs **Interventions ce mois** (une réservation = une intervention si assignée en **c1** ou **c2**), **Prochaine intervention** (**v4.10.0** — renommé depuis **Prochain départ** ; mêmes libellés que le KPI calendrier *sans intervenante* : **Aujourd’hui**, **Demain**, **J-*n***, **Aucun**), **Heures totales ce mois** (somme **`cleaningReports`** pour ce prénom uniquement si **`assignSig`** correspond à l’assignation actuelle — comme **`cleaningReportViewFor`** sur **`index.html`**). **v4.10.0** : les trois KPIs et le tableau d’historique se basent sur la **date effective** (**`menageEffectiveDate`** — date de ménage décalée si elle existe, sinon date de départ) plutôt que sur la date de départ brute, pour qu’une intervention décalée compte sur le **mois où le ménage a réellement lieu**. Tableau d’historique du mois : colonne **« Date ménage »** (**jj/mm/aa**, date effective), studio **S1** / **S2**, heures ou **Non renseigné**, trié par date effective décroissante.
...
- **Navigation latérale** (ordre du haut vers le bas) — une **vue pleine page** par entrée, sans sous-menus :
  ...
  2. **Procédure** — **Procédures & préparation studio** (étapes par studio, photos Storage + texte RTDB, suggestions orange en tête de liste, moyenne des notes par étape ; **v4.5.4** : traçabilité affichée sous les champs texte de chaque étape, copie d’étape avec report / mise à jour des métadonnées), voir § `/procedures` et `/procedureSuggestions` ci-dessus. Halo animé (**`nav-pending-pulse`**) sur le lien menu **« Procédure »** lorsqu’il existe au moins une entrée **`procedureSuggestions`** pour l’org (**`paintProcedureSuggestionsNav`**).
  3. **Comptes** — authentification, **Org. par défaut**, Telegram, liste des comptes, **journal d’activité**.
  4. **Organisation** — **Studios** (noms affichés S1 / S2 sur le calendrier) ; **v4.9.9** : **`adminPersistedStudioCount`** (taille **`config/studioNames`** côté Firebase) — **« Demande de suppression »** **`disabled`** + style grisé pour les studios **non encore enregistrés** (indices **≥** compteur jusqu’à **Enregistrer**) ; **+ Ajouter un studio** : plusieurs ajouts **en local** avant sauvegarde (liste construite depuis les champs + **`A.studioNames`**, sans **`get`** serveur à chaque clic).
  ...
  7. **Résa** (**v4.9.2**, UX **v4.9.9**, alertes **v4.9.10**) — réservations du mois (même périmètre que le planning) : tableau **Arrivée / Nuits / Studio / Voyageurs** ; **v4.9.9** : en-tête **« 📅 Résa »** sur une ligne avec le sélecteur de mois (libellé **mois + année** sur le bouton central) ; **filtre d’affichage par studio** (icône à côté de la colonne **Studio** : cases à cocher, toutes cochées par défaut, **`sessionStorage`** par org **`menage_admin_resa_studio_hidden_v1__{orgId}`**) ; enregistrement **voyageurs** via **`data-resa-uid`** sur la ligne (compatible lignes masquées). **v4.9.10** : **halos menu** (**`nav-pending-pulse`**) — lien **« Réservations »** si au moins une réservation du **mois civil actuel** (date du navigateur) chevauche un départ sans **voyageurs** valides (**`adminResaVoyageursCurrent`**, 1–8) ; halo sur la **colonne Arrivée** uniquement (**`admin-resa-pending-voyageurs`**) lorsque le **mois affiché** dans cette vue est le **mois civil actuel** et que la ligne n’a pas de voyageurs renseignés — **sans** halo sur le **select Voyageurs**.
- **Hashes d’URL** (optionnels) : **`#dashboard`** (équivalent page d’accueil admin), **`#procedures`**, **`#comptes`**, **`#organisation`**, **`#legend`**, **`#indisponibilites`**, **`#reservations`** ; **`#general`** est encore accepté et équivalent à **`#dashboard`** ; sans hash, la vue par défaut est le **Dashboard**.
...
- **Modale « Nouveau compte »** : les champs **Prénom**, **Nom** et **Email** ont une **bordure orange** pour signaler qu’ils servent à la **détection de doublon** dans **`/accounts`** global ; en cas de doublon, une **modale intégrée** propose d’**associer** le compte existant à l’org (plus de **`confirm()`** navigateur).
...
- **Historique des interventions** : tableau avec date courte **jj/mm/aa**, studio **S1** / **S2**, intervenantes, et colonne **Heures** (somme des durées saisies pour la ligne, vide si aucune) ; mise à jour si **`cleaningReports`** change. **v4.10.0** : filtre « déjà passé », regroupement par mois et tri/affichage de la date basés sur la **date effective** (**`menageEffectiveDate`** — voir § **`/assignments`**) plutôt que la date de départ — une intervention décalée n’apparaît dans l’historique qu’une fois le ménage réellement passé, et est classée dans le mois où il a eu lieu. Libellé de groupe **« … intervention(s) »** (au lieu de **« … départ(s) »**).
- **Export Excel** — feuille **Détail interventions** : colonnes dans l’ordre **Date ménage** (**v4.10.0** — renommée depuis **Date départ**, valeur = date effective), **Studio**, **Intervenante 1**, **Intervenante 2**, **Intervenante 1 durée**, **Intervenante 2 durée**, **Commande**, **Commentaire** (durées au format Excel `[h]:mm`, tri par date effective). Feuille **Récap par intervenante** : colonne **Total heures** (somme sur la période exportée, même format). **v4.10.0** : le filtre du mois sélectionné (**`exportSelected`**) et le filtre « historique complet = déjà passé » (**`exportAll`**) utilisent également la date effective, comme les statistiques du dashboard (**`renderStats`** — cartes **Total** / **Assignés** / **Manquants** + compteur par intervenante, mois basé sur la date effective).
...
- **Ménage décalé (v4.10.0)** : le **déclenchement** reste inchangé (toujours le jour réel du départ, `r.end === today`) — seul le **texte** du message change si l’assignation porte une **`menageDate`** différente de la date de départ : la ligne intervenante affiche **« 🧹 Intervenante : Prénom (décalé au jj/mm/aaaa) »** au lieu de **« 🧹 Intervenante : Prénom »**.
...
- **`nextIntervenante`** (`onRequest`, GET — **ajoutée en Septembre 2026, hors cycle de version de l'app**, aucune modification des 4 pages HTML) : endpoint **public mais sécurisé par clé partagée** (`?key=...`), **lecture seule**, sans aucun impact sur l'app ni sur les autres Cloud Functions. Pour un studio donné de l'org "tesson", calcule pour chaque réservation du studio sa **date effective** (`menageDate` de son assignation si un décalage existe, sinon sa date de départ réelle) et renvoie celle dont la date effective future est la plus proche, ainsi que les intervenantes assignées (`assignments/{uid}.c1`/`c2`).
  ...
  - **Bug corrigé après mise en prod (v1)** : la version initiale cherchait le départ juste avant la *prochaine arrivée*, ce qui retombait sur un départ déjà passé quand un studio restait vide un moment (mauvaise assignation renvoyée). Corrigé en cherchant simplement le **prochain départ** (`end >= aujourd'hui`), indépendamment de la prochaine arrivée.
  - **Ménage décalé (v4.10.0)** puis **bug corrigé après mise en prod (v2, Septembre 2026)** : la première version du support `menageDate` renvoyait la date décalée uniquement si elle appartenait au **prochain départ sélectionné par date de départ brute** (`end >= aujourd'hui`) — un départ déjà passé (`end < aujourd'hui`) dont le ménage décalé n'avait **pas encore eu lieu** était donc ignoré, la fonction sautant directement au départ suivant (mauvaise date **et** mauvaise intervenante renvoyées). Corrigé en sélectionnant le prochain ménage sur la **date effective de chaque réservation du studio**, et non plus sur la date de départ seule — voir § **`/assignments`**.
...
Le **nombre** de studios et les **libellés** par défaut sont fixés dans **Super Admin** (vue Organisations). Les **libellés** sont **renommables** dans **admin.html** → vue **Organisation** (champs dynamiques **`#studios-fields`**, enregistrés dans **`/orgs/{orgId}/config/studioNames`**). **Ajout de studio** (**+ Ajouter un studio**) — **v4.9.9** : enchaînement de **plusieurs** ajouts **avant** **Enregistrer** (liste locale, pas un rechargement serveur à chaque clic). **Demande de suppression** (avec annulation) côté **admin** — **v4.9.9** : bouton **inactif** tant que le studio n’existe pas encore dans **`config/studioNames`** (évite une demande sur un index « fantôme »). Traitement côté **Super Admin** (suppression studio / org, nœud **`deletionRequests/studios`**, drapeau global **`/pendingDeletionRequests`**).
...
Dernière étape complétée : v4.10.0 — Ménage décalé (menageDate/menageDateBy
  ...
  de compte.html, les stats/historique/export Excel de admin.html, et le
  texte (pas le déclenchement) de notify-departs.js ; nextIntervenante
  (Cloud Function) renvoie menageDate quand défini ; correctif préexistant
  saveAssignments (_sa/_assignSelf/_removeSelf fusionnaient mal, effaçant
  voyageurs) ; APP_VERSION réaligné sur les 4 HTML (compte.html et
  superadmin.html étaient restées en 4.9.6).
Ajout hors cycle de version (Septembre 2026, sans impact sur les 4 HTML) :
  ...
  functions/index.js — Cloud Functions ci-dessus). Corrigée une 2e fois
  (hors cycle, Septembre 2026) juste après la sortie de v4.10.0 : le choix
  du prochain ménage se base maintenant sur la date effective de chaque
  réservation du studio (menageDate si décalage, sinon date de départ) au
...
**Ménage décalé** — possibilité de décaler le jour du ménage par rapport à la date de départ (studio vide un moment, regroupement de deux ménages), sans casser le fonctionnement existant lorsqu'aucun décalage n'est utilisé.

- **`index.html`** : nouveau champ **`menageDate`** / **`menageDateBy`** sous **`/assignments`** (voir § **`/assignments`**) ; modale **Assigner les intervenantes** — section **Ménage** avec bouton **🧹 Décaler l'intervention de ménage** et sélecteur de date natif borné (`min` = départ, `max` = veille de la prochaine arrivée sur le studio, dates au-delà **grisées** dans le sélecteur + **validation à l'enregistrement** en garde-fou) ; affichage calendrier : bloc départ discret (**`checkout--decaled`**) + bloc dédié **`.resa-block.menage`** sur le jour du ménage décalé, badges intervenantes, filtre **`filterCleaner`**, légende **« 🧹 Ménage décalé »** ; conflit assignation × indispo vérifié sur le bon jour selon décalage ou non.
- **Correctif préexistant** (découvert en auditant les effets de bord avant cette fonctionnalité) : **`_sa`**, **`_assignSelf`**, **`_removeSelf`** reconstruisaient l'objet d'assignation au lieu de le fusionner, effaçant silencieusement des champs comme **`voyageurs`** (saisi séparément depuis **`admin.html`**) à chaque modification d'assignation. Corrigé par fusion systématique via **`rawAssignmentObj(key)`** — préserve tous les champs existants, y compris ceux inconnus de la page.
- **`compte.html`** : KPI **Prochaine intervention** (renommé depuis **Prochain départ**) et **Interventions ce mois** + tableau d'historique (colonne **« Date ménage »**) basés sur la **date effective** (**`menageEffectiveDate`**) plutôt que la date de départ brute.
- **`admin.html`** : même bascule sur la **date effective** pour les statistiques du dashboard (**Total** / **Assignés** / **Manquants** + compteur par intervenante), l'**Historique des interventions** (filtre « déjà passé », regroupement par mois, tri, date affichée) et l'**export Excel** (colonne **« Date ménage »** renommée depuis **« Date départ »**, filtres mois sélectionné et historique complet).
- **`notify-departs.js`** : déclenchement inchangé (toujours le jour réel du départ) ; texte adapté si décalage — **« 🧹 Intervenante : Prénom (décalé au jj/mm/aaaa) »**.
- **`functions/index.js`** — **`nextIntervenante`** (hors cycle de version, voir § **functions/index.js**) : renvoie **`menageDate`** à la place de la date de départ quand il est défini.
- **`styles.css`** : styles **`input[type=date]`** partagés (`.modal-field`), y compris surcharges **`::-webkit-datetime-edit*`** pour aligner l'apparence du sélecteur natif sur les autres champs de formulaire.
- **`APP_VERSION`** : **4.10.0** dans les 4 HTML (alignement — **`compte.html`** et **`superadmin.html`** étaient restées en **4.9.6**, non touchées par un cycle de version depuis).

### v4.9.10 — Mai 2026

- **`admin.html` — Menu « Réservations »** : classe **`has-pending-voyageurs`** + même animation **`nav-pending-pulse`** que **Procédure** lorsqu’au moins une réservation **chevauche le mois civil actuel** (date système navigateur) et n’a pas de **`voyageurs`** valides (1–8) dans **`assignments`** (**`paintReservationsVoyageursNav`**) ; mise à jour sur **`onValue(assignments)`** et **`onValue(reservations)`** ainsi qu’avec **`paintProcedureSuggestionsNav`** au chargement **`renderAdminContent`**.
- **`admin.html` — Vue Résa (tableau)** : pour le **mois affiché = mois civil actuel** uniquement, la **cellule « Arrivée »** entoure la date (**`admin-resa-pending-voyageurs`**) lorsque les voyageurs manquent ; **sans** halo sur le **select Voyageurs**.
- **`index.html` — Pastille orange titre (`#pending-suggestions-pip`)** : même logique **voyageurs mois civil actuel** + suggestions procédure existantes (**`indexHasMissingVoyageursCurrentMonth`**, **`indexResaVoyageursCurrent`**), visible pour **admin** ou **sans auth** ; **`paintSuggestionsPip`** également sur **`assignments`** / **`reservations`**.
- **`index.html` — Planning** : conflit **départ jour J** (**`getCheckouts`** + filtres **`filterCleaner`** alignés sur l’affichage départs) ∩ **indispo même date locale** ⇒ **`day-cell--assign-indispo-warn`** (bordure rouge, titre au survol) **sans** obliger le filtre **« Indispo »**.
- **`APP_VERSION`** : **4.9.10** dans **`index.html`** et **`admin.html`**.

### v4.9.9 — Mai 2026

- **`admin.html` — Vue Résa (`#reservations`)** : filtre d’**affichage** par studio (bouton à côté de l’en-tête **Studio** : cases à cocher, toutes cochées par défaut ; préférence **`sessionStorage`** par org **`menage_admin_resa_studio_hidden_v1__{orgId}`**) ; au moins un studio reste visible ; en-tête compact **« 📅 Résa »** + sélecteur de mois sur une ligne (libellé **mois + année** sur le bouton central) ; suppression du paragraphe d’aide redondant ; **`adminResaVoyageursChange`** basé sur **`data-resa-uid`** sur la ligne du tableau.
- **`admin.html` — Vue Organisation (Studios)** : **`adminPersistedStudioCount`** aligné sur **`config/studioNames`** (listener **`config`** + **`saveNames`**) — **« Demande de suppression »** désactivée et grisée pour les studios **non encore enregistrés** (évite **`deletionRequests/studios/{i}`** sur un index inexistant en base) ; **+ Ajouter un studio** : plusieurs clics **avant Enregistrer** en concaténant la liste locale (valeurs des champs + **`A.studioNames`**) au lieu d’un **`get`** serveur à chaque ajout.
- **`APP_VERSION`** : **4.9.9** dans **`admin.html`** (les autres pages HTML inchangées sur cette livraison).

### v4.9.6 — Avril 2026

- **Super Admin — Suppression d'organisation** : suppression des comptes Firebase Authentication orphelins (sans autre org) lors de la cascade de suppression d'org (`performOrgCascadeDelete`) ; modale de confirmation refactorisée en 2 étapes (saisie du libellé puis re-auth séparée) ; description de confirmation personnalisée avec le nom de l'organisation.
- **Super Admin — Modale re-auth générique** : `buildReauthModalHTML()` dans `fonctions.js` + `openReauthModal({ description, onConfirm, onCancel })` dans `superadmin.html` — les 3 actions sensibles (suppression org, suppression studio, modification iCal) passent par cette modale unique ; affichage Google ou mot de passe selon le provider actif ; feedback visuel après re-auth Google (bouton vert ✓, libellé "Valider").
- **Super Admin — Invitation** : `telegramEnabled: false` ajouté dans `adminConfig` à la création d'org via `inviteToken.consume` (Cloud Function).
- **Calendrier (`index.html`)** : fin des allers-retours sablier / planning (retour depuis l’admin, changement d’organisation) ; `showApp()` uniquement quand les données org sont prêtes (`fbReady`) ; `resolveSessionFromFirebaseUser` ne déclenche plus `showApp()` (délégation aux appelants) ; `await checkAuth()` dans `routeAfterFirebaseReady` ; repli connexion email / Google si `fbReady` déjà vrai.
- **`superadmin.html`** : sablier pendant la connexion et le chargement après auth ; premier affichage sans flash du formulaire login quand une session Firebase autorisée est déjà présente ; repli **timeout** (~28 s) et balise **`noscript`** si JavaScript est désactivé.
- **`admin.html` (vue Organisation — Studios)** : modale d’aide **« ? »** (où trouver l’URL iCal sur Airbnb) ; liste des studios en **cartes** (`.studio-card`) pour une meilleure lisibilité sur mobile.
- **`invitation.html` (étape 5 — URLs iCal)** : même contenu d’aide en **modale locale** (`invite-help-*`, visibilité `is-visible`).
...
- **Super Admin** — nouvelle vue **Invitations** dans **`superadmin.html`**
- **Génération** de liens uniques (**24** caractères base64url, **144** bits d’entropie)
- **Schéma RTDB** **`/inviteTokens/{token}`** (création, expiration, statut, métadonnées d’usage réservées)
- **Modales** : création (validité **7 / 30 / 90** jours), affichage du lien après génération, **révocation**, **suppression définitive** des liens non actifs (utilisés / révoqués / expirés)
- **Envoi guidé** : bouton 💬 sur les liens actifs ouvrant une modale avec champ destinataire optionnel et message d'invitation pré-rédigé éditable, prêt à coller dans WhatsApp / SMS / email
- **Badges** : **Actif** / **Expiré** (affichage) / **Utilisé** / **Révoqué**
- **Préparation** de la fonctionnalité **invitation par token** — page **`invitation.html`** et consommation du token (Cloud Function) **à venir**
...
- **Admin — Vue Réservations** : nouvelle section dans le menu burger listant les réservations du mois avec navigation mois par mois, colonnes Arrivée / Nuits / Studio / Voyageurs (liste déroulante 1–8, sauvegardée dans `assignments/{uid}/voyageurs`) — UX et filtre par studio : **v4.9.9** (voir § **Historique — v4.9.9**)
...
- **Correction `accEditFindGlobal`** : alignement `admin.html` sur `superadmin.html` — filtre sur `row.prenom` **ou** `row.name` pour le rapprochement compte org ↔ `/accounts`
- **`compte.html` — changement de mot de passe** : refonte du flux — si l'utilisateur est connecté via Firebase Auth, utilisation de `reauthenticateWithCredential` + `updatePassword` (SDK client) comme chemin principal ; mise à jour RTDB en bonus ; suppression du message "contacter l'administrateur" pour les comptes avec `authUid`
- **Sécurité CF `adminAuth`** : ⚠️ TODO — la Cloud Function n'impose pas encore de vérification de rôle côté serveur (tout utilisateur Firebase Auth authentifié peut l'appeler). À traiter en v4.10.0.

### v4.9.0 — Avril 2026

- **Firebase Authentication** (email / Google) sur **`index.html`**, **`admin.html`**, **`compte.html`**, **`superadmin.html`**.
- **Règles RTDB** durcies (**`auth != null`**) avec exception lecture publique **`/organizations`**.
- **Emails réels** sur tous les comptes (alignement Auth / RTDB via scripts de migration one-shot).
- **Nettoyage** code mort et incohérences (handlers obsolètes, constantes inutilisées, garde anti double **`logActivity('login')`**, etc.).
...
- **`admin.html`** — modale de création de compte : **bordures orange** sur les champs **Prénom** / **Nom** / **Email** (champs utilisés pour la détection de doublon).
- **`admin.html`** — remplacement du **`confirm()`** navigateur par une **modale intégrée** (**`#acc-modal-confirm-assoc`**) pour l’association d’un compte existant, alignée sur le comportement de **`superadmin.html`**.

### v4.8.1 — Avril 2026
- **Nettoyage Firebase** : suppression des **7** nœuds racine orphelins post-migration v4.0.0 (**`/activityLog`**, **`/activityLogs`**, **`/lastSync`**, **`/config`**, **`/reservations`**, **`/assignments`**, **`/adminConfig`**) via script one-shot **`cleanup-legacy-root.js`**.
- **Couleurs intervenantes** : nouveau champ **`order`** (0-based) dans **`/accounts/{id}`** ; **`refreshCleanerList()`** trie par **`order`** ; **`superadmin.html`** aligne ses avatars sur **`CLEANER_COLORS`** ; nouveaux comptes reçoivent **`order` = max + 1** automatiquement via **`admin.html`** et **`superadmin.html`** ; script one-shot **`set-accounts-order.js`** pour les comptes existants.
- **Fix flash écran « Organisation non configurée »** : chaînage **`refreshCleanerList().then(afterFbListener)`** dans **`onValue`** **`adminConfig`** (**`index.html`**).
- **UX** : écran **« Changement d’organisation »** affiché lors du switch d’org depuis la **topbar** (**`index.html`**).
- **`APP_VERSION` : 4.8.1** dans les quatre fichiers HTML.
...
- **`index.html`**, **`admin.html`**, **`compte.html`**, **`superadmin.html`** — **`APP_VERSION` : 4.7.0**. Connexion calendrier : **`<select id="login-name">`** alimenté depuis **`/accounts`** dans **`showLogin()`**, puis **`doLogin`** (global SHA-256 puis legacy) ; cold start **sans** écran org obligatoire (**`menage_org_v1`** = première org ou **`tesson`**). Écran **`#org-first-screen`** conservé mais **non** branché au bootstrap (`showOrgFirst` non appelée).
- **`superadmin.html`** — Panneau comptes par org : lecture **`/accounts`**, correspondance **`name`** / **`prenom`**, écritures ciblées sur **`accounts/{id}/...`** + tableau legacy.
- **`compte.html`** — Déverrouillage / MDP : toujours via **`adminConfig.accounts`** (legacy) ; utiliser **`patch-legacy-accounts.js`** si besoin d’alignement avec **`/accounts`**.
- **Scripts** — **`init-accounts.js`** (compte de test **`/accounts`**), **`check-accounts.js`** (rapport cohérence), **`patch-legacy-accounts.js`** (réalignement legacy depuis global).
- **Retrait dépôt** — **`init-accounts-migration.js`** (one-shot déjà exécuté, mots de passe en clair dans l’ancienne version) : fichier supprimé ; entrée **`init-accounts-migration.js`** dans **`.gitignore`** pour éviter un re-commit accidentel.
- **Supprimé** — **`login-test.html`** (page de test retirée du dépôt).

### v4.6.3 — Avril 2026
- **Demandes de suppression de studio** — **`admin.html`** : bouton **Demande de suppression** / **Annuler** par studio (`/orgs/{orgId}/deletionRequests/studios/{i}`), écriture **`/pendingDeletionRequests`** quand il reste au moins une demande dans une org ; **`superadmin.html`** : pastille **orange** sur la ligne org, annulation de demande, suppression studio après re-auth ; recalcul du drapeau **`/pendingDeletionRequests`** après suppression studio ou **suppression d’organisation** (évite un état « bloqué » côté calendrier).
- **`index.html`** — pastille orange sur **Accueil** (hors app principale) si **`/pendingDeletionRequests`** ; pastille sur **Planning Ménage** (admin / sans auth) si **`procedureSuggestions`** non vide ; listener **`onValue`** sur **`procedureSuggestions`** ; **`APP_VERSION` : 4.6.3**.
- **`admin.html`** — halo pulsant sur le lien **Procédure** du menu latéral si suggestions en attente ; édition **URL iCal** par studio (modale, respect du **`locked`** imposé par le Super Admin) ; **+ Ajouter un studio** ; **`APP_VERSION` : 4.6.3**.
- **`superadmin.html`** — **iCal** : champ **`locked`** conservé à l’enregistrement ; cadenas **🔒 / 🔓** dans la modale (état courant) ; libellé bouton **🔑** atténué si verrouillé ; **`telegramEnabled: false`** par défaut à la **création** d’org ; **liste orgs** : réinitialisation **`studiosHydrated`** après **`renderOrgList()`** (réouverture accordéon) ; **`setOrgListLoading`** (⏳ + désactivation **+ Créer**) pendant création / suppression d’org ; renommage **✏️** du libellé org ; slug non affiché sur la ligne ; topbar une ligne **Planning Ménage · v** ; message login **non prévu pour smartphone** ; **`APP_VERSION` : 4.6.3**.
- **`compte.html`** — **`APP_VERSION` : 4.6.3**.

### v4.6.2 — Avril 2026
- **`superadmin.html`** — refonte vue **Organisations** : **accordéon** à deux volets (**comptes** / **studios**) avec état **`studiosOpen` / `accountsOpen`** ; ligne d’org : toggles **🔐** / **💬** (`adminConfig`), badge **👤** + panneau comptes inline, **badge étapes** (cache procédures) ; suppression de l’ancien bouton **« X comptes associés »** ; entrées menu **Comptes** et **Paramètres** **masquées** (`display:none`) ; vue par défaut **Organisations** ; **`APP_VERSION` : 4.6.2**.
- **`admin.html`** — retrait de l’UI **authentification**, **Telegram** et **organisation par défaut** sur la vue Comptes (réglages côté Super Admin / RTDB) ; **`APP_VERSION` : 4.6.2**.
- **`index.html`**, **`compte.html`** — **`APP_VERSION` : 4.6.2**.
