...
- **`nextIntervenante`** (`onRequest`, GET — **ajoutée en Septembre 2026, hors cycle de version de l'app**, aucune modification des 4 pages HTML) : endpoint **public mais sécurisé par clé partagée** (`?key=...`), **lecture seule**, sans aucun impact sur l'app ni sur les autres Cloud Functions. Pour un studio donné de l'org "tesson", cherche directement dans `/orgs/{orgId}/reservations` le **prochain départ à venir** (`end >= aujourd'hui`, le plus proche, **triage indépendant du ménage décalé** — voir ci-dessous) et renvoie la date à laquelle le ménage doit avoir lieu ainsi que les intervenantes assignées (`assignments/{uid}.c1`/`c2`).
  ...
  - **Ménage décalé (v4.10.0)** : si l'assignation du prochain départ porte une **`menageDate`**, c'est **cette date** qui est renvoyée dans `date` (au lieu de la date de départ). Le **choix du prochain départ** reste basé sur la date de départ réelle (`end`), pas sur la date effective — simplification volontaire : un décalage ne doit **jamais** dépasser la prochaine arrivée sur le studio (garde-fou UI + validation à l'enregistrement côté **`index.html`**, voir § **`/assignments`**), donc le départ le plus proche reste toujours le bon candidat en pratique.
