...
- **Navigation latérale** (ordre du haut vers le bas) — une **vue pleine page** par entrée, sans sous-menus :
  ...
  7. **Résa** (**v4.9.2**, UX **v4.9.9**) — réservations du mois (même périmètre que le planning) : tableau **Arrivée / Nuits / Studio / Voyageurs** ; **v4.9.9** : en-tête **« 📅 Résa »** sur une ligne avec le sélecteur de mois (libellé **mois + année** sur le bouton central) ; **filtre d’affichage par studio** (icône à côté de la colonne **Studio** : cases à cocher, toutes cochées par défaut, **`sessionStorage`** par org **`menage_admin_resa_studio_hidden_v1__{orgId}`**) ; enregistrement **voyageurs** via **`data-resa-uid`** sur la ligne (compatible lignes masquées).
...
Version : 4.9.9
Dernière version stable : v4.9.9
Dernière étape complétée : v4.9.9 — Admin : vue Résa (filtre par studio,
  en-tête compact), vue Organisation (brouillon studios + multi-ajout)
...
Fichiers : index.html (v4.9.6), admin.html (v4.9.9), compte.html (v4.9.6),
